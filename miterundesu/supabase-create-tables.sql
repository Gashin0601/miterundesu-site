...
-- ポリシー削除して再作成
DROP POLICY IF EXISTS "Service role can do everything on contacts" ON public.contacts;
...
-- ポリシー削除して再作成
DROP POLICY IF EXISTS "Service role can do everything on store_applications" ON public.store_applications;
...
-- ポリシー削除して再作成
DROP POLICY IF EXISTS "Service role can do everything on press_applications" ON public.press_applications;
...
-- 完了
-- ============================================
-- テーブル、インデックス、ポリシー、トリガーの作成が完了しました。
